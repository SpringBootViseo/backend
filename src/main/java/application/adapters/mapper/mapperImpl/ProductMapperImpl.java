...
public class ProductMapperImpl implements ProductMapper {
    ...
    public Product productToProductEntity(ProductEntity productEntity) {
        ...
        return  new Product(productEntity.getId(),productEntity.getName(),productEntity.getMarque(),productEntity.getLinkImg(),productEntity.getDescription(),productEntity.getImages(),productEntity.getUnitQuantity(),productEntity.getReductionPercentage(), productEntity.getPreviousPrice(), productEntity.getCurrentPrice(), category);
    }
    ...
    public ProductEntity productEntityToProduct(Product product) {
        ...
        return new ProductEntity(product.getId(),product.getName(),product.getMarque(),product.getLinkImg(),product.getDescription(),product.getImages(),product.getUnitQuantity(),product.getReductionPercentage(), product.getPreviousPrice(), product.getCurrentPrice(), categoryEntity);
    }
    ...
    public List<Product> categoryToDocument(MongoCollection<Document> collection) {
        ...
        for(Document doc:collection.find()){
            ...
            productEntityList.add(new ProductEntity(doc.get("_id", UUID.class),doc.getString("name"),doc.getString("marque"),doc.getString("linkImg"),doc.getString("description"),doc.getList("Images", String.class),null,doc.getLong("reductionPercentage"),doc.getLong("previousPrice") , doc.getLong("currentPrice"),categoryEntity ));
        }
    ...
    public Product productToProductDto(ProductDTO productDTO) {
        ...
        return  new Product(productDTO.getId(),productDTO.getName(),productDTO.getMarque(),productDTO.getLinkImg(),productDTO.getDescription(),productDTO.getImages(),productDTO.getUnitQuantity(),productDTO.getReductionPercentage(), productDTO.getPreviousPrice(), productDTO.getCurrentPrice(), category);
    ...
    public ProductDTO productDtoToProduct(Product product) {
        ...
        return  new ProductDTO(product.getId(),product.getName(),product.getMarque(),product.getLinkImg(),product.getDescription(),product.getImages(),product.getUnitQuantity(),product.getReductionPercentage(), product.getPreviousPrice(), product.getCurrentPrice(), category);
